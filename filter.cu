...
{ 
  // map from blockIdx to pixel position
	...
	int index = x + y * imagesizex;
  	...
  	__shared__ unsigned char images[blocksize * blocksize * 3];

	images[bindex*3+0] = image[index*3+0];
	images[bindex*3+1] = image[index*3+1];
	images[bindex*3+2] = image[index*3+2]; 
...
// Filter kernel (simple box filter)
		...
		for(dy=-kernelsizey;dy<=kernelsizey;dy++)
			...
			{
				// Use max and min to avoid branching!
				int yy = min(max(threadIdx.y+dy, 0), imagesizey-1);
				int xx = min(max(threadIdx.x+dx, 0), imagesizex-1);
				
				sumx += images[((yy)*blockDim.x+(xx))*3+0];
				sumy += images[((yy)*blockDim.x+(xx))*3+1];
				sumz += images[((yy)*blockDim.x+(xx))*3+2];
			}
		out[(index+x)*3+0] = sumx/divby;
		out[(index+x)*3+1] = sumy/divby;
		out[(index+x)*3+2] = sumz/divby;
	}
}
